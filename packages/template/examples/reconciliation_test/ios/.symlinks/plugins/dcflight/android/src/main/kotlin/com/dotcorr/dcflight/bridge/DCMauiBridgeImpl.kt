...
class DCMauiBridgeImpl private constructor() {
    ...
    fun updateView(viewId: String, propsJson: String): Boolean {
        return try {
            ...
            // ⭐ FIX FLICKERING: Only requestLayout if layout props changed
            // This prevents flickering for all components (Text, Image, Button, etc.)
            // when only content/non-layout props change (e.g., text content, image source, button title)
            ...
            if (layoutPropsChanged) {
                // Layout props changed - need full layout recalculation
                view.invalidate()
                view.requestLayout()
            } else {
                // Only non-layout props changed (content, colors, etc.) - just redraw, no layout
                // This prevents flickering when updating text content, image source, button title, etc.
                view.invalidate()
            }
    ...
    fun commitBatchUpdate(operations: List<Map<String, Any>>): Boolean {
        ...
        try {
            ...
            // ⭐ CRITICAL: Ensure all views are visible after layout (same as iOS)
            // This prevents white screen on initial render
