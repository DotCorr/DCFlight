...
@objc class DCMauiBridgeImpl: NSObject {
    ...
    @objc func commitBatchUpdate(updates: [[String: Any]]) -> Bool {
        ...
        do {
            ...
            // Note: This must be synchronous to prevent white screen, but should be fast
            let layoutSuccess = YogaShadowTree.shared.calculateAndApplyLayout(
                width: windowBounds.width,
                height: windowBounds.height
            )
            
            // ⭐ CRITICAL: Ensure root view is visible after layout to prevent white screen
            // This is especially important after reverting pooling
            if let rootView = views["root"] {
                rootView.isHidden = false
                rootView.alpha = 1.0
                // Ensure root view has proper frame
                if rootView.frame == .zero {
