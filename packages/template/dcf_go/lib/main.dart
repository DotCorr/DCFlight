  ...
  DCFComponentNode render() {
    ...
    if (controllerRef.current == null) {
      ...
      controllerRef.current!.onLoaded = () async {
        ...
        final byteData = await surfaceImage.toByteData(
          ...
          children: [
            ...
            DCFView(
              ...
              children: [
                DCFText(
                  content:
                      "Rotation Speed: ${(rotationSpeed.state * 100).toStringAsFixed(0)}%",
                  ...
                  children: [
                    DCFButton(
                      onPress: (DCFButtonPressData data) {
                        rotationSpeed.setState(
                          (rotationSpeed.state - 0.01).clamp(0.0, 0.2),
                        );
...
Future<ui.Image> _createDefaultSphereImage() async {
  ...
  final oceanPaint =
      ui.Paint()
        ..shader = ui.Gradient.radial(
          center,
          radius,
          [const ui.Color(0xFF4A90E2), const ui.Color(0xFF2E5C8A)],
          [0.0, 1.0],
        );
  ...
  final path1 = ui.Path()..addOval(ui.Rect.fromLTWH(150, 100, 120, 80));
  canvas.drawPath(path1, landPaint);

  final path2 = ui.Path()..addOval(ui.Rect.fromLTWH(250, 200, 100, 90));
  canvas.drawPath(path2, landPaint);

  final path3 = ui.Path()..addOval(ui.Rect.fromLTWH(100, 300, 140, 100));
  canvas.drawPath(path3, landPaint);

  final path4 = ui.Path()..addOval(ui.Rect.fromLTWH(300, 350, 110, 70));
  canvas.drawPath(path4, landPaint);

  // Add some cloud-like white areas
  final cloudPaint = ui.Paint()..color = const ui.Color(0x88FFFFFF);
  final cloudPath1 = ui.Path()..addOval(ui.Rect.fromLTWH(180, 150, 60, 40));
  canvas.drawPath(cloudPath1, cloudPaint);

  final cloudPath2 = ui.Path()..addOval(ui.Rect.fromLTWH(280, 250, 50, 35));
