...
static const CGFloat DCFViewBorderThreshold = 0.001;

BOOL DCFBorderInsetsAreEqual(UIEdgeInsets borderInsets)
{
  return
  ABS(borderInsets.left - borderInsets.right) < DCFViewBorderThreshold &&
  ABS(borderInsets.left - borderInsets.bottom) < DCFViewBorderThreshold &&
  ABS(borderInsets.left - borderInsets.top) < DCFViewBorderThreshold;
}

BOOL DCFCornerRadiiAreEqual(DCFCornerRadii cornerRadii)
{
  return
  ABS(cornerRadii.topLeft - cornerRadii.topRight) < DCFViewBorderThreshold &&
  ABS(cornerRadii.topLeft - cornerRadii.bottomLeft) < DCFViewBorderThreshold &&
  ABS(cornerRadii.topLeft - cornerRadii.bottomRight) < DCFViewBorderThreshold;
}
...
DCFCornerInsets DCFGetCornerInsets(DCFCornerRadii cornerRadii,
                                   UIEdgeInsets edgeInsets)
{
  return (DCFCornerInsets) {
    {
...
static void DCFPathAddEllipticArc(CGMutablePathRef path,
                                       ...
                                       const CGAffineTransform *transform)
{
  ...
  DCFPathAddEllipticArc(path, transform, (CGPoint){
...
static void DCFEllipseGetIntersectionsWithLine(CGRect ellipseBounds,
                                               CGPoint lineStart,
                                               CGPoint lineEnd,
...
NS_INLINE BOOL DCFCornerRadiiAreAboveThreshold(DCFCornerRadii cornerRadii) {
  return (cornerRadii.topLeft > DCFViewBorderThreshold ||
    cornerRadii.topRight > DCFViewBorderThreshold      ||
    cornerRadii.bottomLeft > DCFViewBorderThreshold    ||
    cornerRadii.bottomRight > DCFViewBorderThreshold);
}

static CGPathRef DCFPathCreateOuterOutline(BOOL drawToEdge, CGRect rect, DCFCornerRadii cornerRadii) {
  if (drawToEdge) {
    return CGPathCreateWithRect(rect, NULL);
  }

  return DCFPathCreateWithRoundedRect(rect, DCFGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
}

static CGFloat DCFGetAlphaFromColor(CGColorRef color) {
  if (color == NULL) {
    return 0.0;
  }
  const CGFloat *components = CGColorGetComponents(color);
  return CGColorGetNumberOfComponents(color) == 4 ? components[3] : 1.0;
}

static CGContextRef DCFUIGraphicsBeginImageContext(CGSize size, CGColorRef backgroundColor, BOOL hasCornerRadii, BOOL drawToEdge) {
  const CGFloat alpha = DCFGetAlphaFromColor(backgroundColor);
  const BOOL opaque = (drawToEdge || !hasCornerRadii) && alpha == 1.0;
  UIGraphicsBeginImageContextWithOptions(size, opaque, 0.0);
  return UIGraphicsGetCurrentContext();
}

static UIImage *DCFGetSolidBorderImage(DCFCornerRadii cornerRadii,
                                       CGSize viewSize,
                                       ...
                                       BOOL drawToEdge)
{
  const BOOL hasCornerRadii = DCFCornerRadiiAreAboveThreshold(cornerRadii);
  const DCFCornerInsets cornerInsets = DCFGetCornerInsets(cornerRadii, borderInsets);
  ...
  CGContextRef ctx = DCFUIGraphicsBeginImageContext(size, backgroundColor, hasCornerRadii, drawToEdge);
  const CGRect rect = {.size = size};
  CGPathRef path = DCFPathCreateOuterOutline(drawToEdge, rect, cornerRadii);
  ...
  CGPathRef insetPath = DCFPathCreateWithRoundedRect(UIEdgeInsetsInsetRect(rect, borderInsets), cornerInsets, NULL);

  CGContextAddPath(ctx, insetPath);
  CGContextEOClip(ctx);

  BOOL hasEqualColors = DCFBorderColorsAreEqual(borderColors);
  if ((drawToEdge || !hasCornerRadii) && hasEqualColors) {
    ...
    if (cornerInsets.topLeft.width > 0 && cornerInsets.topLeft.height > 0) {
      CGPoint points[2];
      DCFEllipseGetIntersectionsWithLine((CGRect){
...
static UIImage *DCFGetDashedOrDottedBorderImage(NSString *borderStyle,
                                                ...
                                                BOOL drawToEdge)
{
  NSCParameterAssert([borderStyle isEqualToString:@"dashed"] || [borderStyle isEqualToString:@"dotted"]);

  if (!DCFBorderColorsAreEqual(borderColors) || !DCFBorderInsetsAreEqual(borderInsets)) {
    NSLog(@"Unsupported dashed / dotted border style");
    return nil;
  }
  ...
  const BOOL hasCornerRadii = DCFCornerRadiiAreAboveThreshold(cornerRadii);
  CGContextRef ctx = DCFUIGraphicsBeginImageContext(viewSize, backgroundColor, hasCornerRadii, drawToEdge);
  const CGRect rect = {.size = viewSize};

  if (backgroundColor) {
    CGPathRef outerPath = DCFPathCreateOuterOutline(drawToEdge, rect, cornerRadii);
  ...
  CGPathRef path = DCFPathCreateWithRoundedRect(pathRect, DCFGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);

  CGFloat dashLengths[2];
  dashLengths[0] = dashLengths[1] = ([borderStyle isEqualToString:@"dashed"] ? 3 : 1) * lineWidth;
                           ...
                           BOOL drawToEdge)
{
  ...
  if ([borderStyle isEqualToString:@"solid"] || borderStyle == nil || [borderStyle length] == 0) {
    return DCFGetSolidBorderImage(cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
  } else if ([borderStyle isEqualToString:@"dashed"] || [borderStyle isEqualToString:@"dotted"]) {
    return DCFGetDashedOrDottedBorderImage(borderStyle, cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
  }
